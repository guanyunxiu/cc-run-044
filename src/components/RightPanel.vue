...
const tab = ref<'edit' | 'plan' | 'stats'>('plan')
...
<template>
  <div class="panel">
    <div class="action-bar">
      ...
      <button @click="exportJson" title="导出场景配置 JSON">导出</button>
    ...
    <div class="tabs">
      <div class="tab" :class="{ active: tab === 'edit' }" @click="tab = 'edit'">
        环境编辑
      </div>
      <div class="tab" :class="{ active: tab === 'plan' }" @click="tab = 'plan'">
        规划参数
      </div>
    ...
    <div class="footer-bar">
      <span class="tip">动态对抗：开启在线重规划后播放</span>
      <span class="ver">UAV Path Sim v2.0 · WebGL2</span>
    </div>
  </div>
</template>
