...
export function checkReplanTriggers(ctx: ReplanContext): TriggerResult {
  ...
  // 2) 突发/移动威胁进入预警范围（场强或距离）
  if (triggers.onThreatApproach) {
    for (let i = 0; i < horizon.length; i += 2) {
      const s = horizon[i]
      for (const e of env.dynamics) {
        const st = env.stateAt(e, s.time)
        if (!st.active) continue
        const range = e.kind === 'threat' ? e.threatRadius : e.radius
        const d = Math.hypot(
          s.position.x - st.position.x,
          s.position.z - st.position.z
        )
        if (d < range + triggers.warnDistance) {
          return {
            triggered: true,
            reason: 'threat-approach',
            detail: `${e.name} 进入预警范围（${d.toFixed(0)}m）`,
            hazard: { ...st.position }
          }
        }
      }
