...
/** 时变圆柱障碍（动态移动障碍在某时刻的快照或带速度的实时体） */
export interface MovingCylinder {
  position: Vec3
  radius: number
  heightMin: number
  heightMax: number
  /** 水平速度（m/s），用于预测避让时膨胀；缺省为 0 */
  velocity?: Vec3
  /** 关联动态实体 id */
  entityId?: string
}
...
export class Environment {
  ...
  /** 点是否进入动态移动圆柱障碍（含安全距离膨胀） */
  private hitsMoving(p: Vec3, clearance: number): boolean {
    for (const m of this.moving) {
      const planar = Math.hypot(p.x - m.position.x, p.z - m.position.z)
      if (
        planar <= m.radius + clearance &&
        p.y <= m.heightMax + clearance &&
        p.y >= m.heightMin - clearance
      ) {
        return true
      }
    }
  ...
  /**
   * 动态障碍碰撞风险：给定位置 p 与到达该位置的时间 atTime，
   * 按障碍速度线性预测位置后做圆柱碰撞（含 clearance 膨胀）。
   * 返回最近的碰撞障碍；无碰撞返回 null。
   */
  predictedCollision(
    p: Vec3,
    atTime: number,
    ...
    for (const m of this.moving) {
      ...
      const cx = m.position.x + v.x * atTime
      const cz = m.position.z + v.z * atTime
      const planar = Math.hypot(p.x - cx, p.z - cz)
      if (
        planar <= m.radius + clearance &&
        p.y <= m.heightMax + clearance &&
        p.y >= m.heightMin - clearance
  ...
  /**
   * 点到最近动态圆柱表面的净空（米）：<= clearance 表示侵入安全裕度。
   * 返回 {id, distance, entityId}；无动态障碍时 distance=Infinity。
   */
  movingClearance(p: Vec3): {
    distance: number
    entityId?: string
  } {
    let best = Infinity
    ...
    for (const m of this.moving) {
      const planar = Math.hypot(p.x - m.position.x, p.z - m.position.z)
      const dy = Math.max(m.heightMin - p.y, 0, p.y - m.heightMax)
      const dr = Math.max(planar - m.radius, 0)
    ...
    return { distance: best, entityId: bestId }
  }
  ...
  isBlocked(p: Vec3, clearance: number): boolean {
    ...
    if (this.hitsMoving(p, clearance)) return true
    return false
  }
