...
<template>
  <div class="playback">
    <div class="left">
      ...
      <button :disabled="sim.trajectory.length < 2" @click="sim.seek(0)">⏮ 回放</button>
      <button
      ...
      <button
        ...
        @click="replan"
        title="基于当前场景与参数重新执行全局规划（全部无人机）"
      >
        ↻ 重新规划
      </button>
      <label class="mini-chk" title="播放时遇到移动障碍/突发威胁自动局部重规划">
        <input type="checkbox" v-model="sim.replanConfig.enabled" />
        在线重规划
      </label>
    </div>
    ...
    <div class="right">
      <span class="mini" v-if="sim.stats?.algo">{{ ALGO_LABELS[sim.stats.algo] }}</span>
      <span class="mini">航程 {{ coveredDist.toFixed(0) }} m</span>
      <span class="mini">速度 {{ currentSpeed.toFixed(1) }} m/s</span>
      <span class="mini">重规划 {{ sim.replanEvents.length }} 次</span>
    </div>
  </div>
</template>
