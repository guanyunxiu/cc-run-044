...
class FakeWorker {
  ...
  async postMessage(msg: any) {
    const [{ Environment }, { planMission, planMultiMission }, { environmentAt }] =
      await Promise.all([
        import('@/core/environment'),
        import('@/core/planning'),
        import('@/core/dynamicEnv')
      ])
    if (msg.type === 'plan') {
      ...
      queueMicrotask(() =>
        this.onmessage?.({
          data: { type: 'plan-multi-done', results, workerMs: 23.4 }
        } as MessageEvent)
      )
    }
  }
...
describe('Web Worker 规划链路（store.plan）', () => {
  it('Worker 返回规划结果并生成轨迹、代价曲线、统计', async () => {
    ...
    expect(sim.message).toMatch(/规划成功|规划完成/)
